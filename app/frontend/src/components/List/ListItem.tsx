...
const ListItem: React.FC<TListItem> = ({ task, fetchAllTasks }) => {
	...
	const { updatingHandlers, editHandlers } = useContext<ITasksContext | null>(TasksContext) || {};
	...
	return (
		<AccordionItem m="0 8px 0 0" border="none" onMouseEnter={setShowActions.on} onMouseLeave={setShowActions.off}>
			{({ isExpanded }) => (
				<>
					<Flex alignItems="center" w="100%" h={isExpanded ? "fit-content" : "36px"} justifyContent="space-between">
						{isUpdating === task.id ? (
							<Box display="flex" alignItems="center" gap="8px" verticalAlign="middle">
								<CircularProgress
									isIndeterminate
									color="orange.500"
									display="flex"
								...
								<Flex alignItems="center" gap="8px">
									...
									{task.status === "COMPLETED" && (
										<Text fontSize="0.5rem" fontStyle="italic" color="gray.500">
											{(task.conclusionDate && `Completed at ${format(new Date(task?.conclusionDate), "Pp")}`) || ""}
										</Text>
									)}
								</Flex>
							</Box>
					...
					{task.description && (
						<AccordionPanel alignSelf="center" alignItems="flex-start" display="flex" flexDirection="row" p={0}>
							<Divider borderColor="orange.400" borderWidth={1} h={4} m="0px 8px" orientation="vertical" />
