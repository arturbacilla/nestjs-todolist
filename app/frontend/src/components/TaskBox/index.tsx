...
const TaskBox: React.FC<TaskBoxProps> = ({
	...
	const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		...
		return requestPostPut(endpoint, { title, description, authorId }, token, type === "edit" ? "put" : "post")
			.then(() => {
				fetchAllTasks();
				handleClose(e);
				const message = `Task '${type === "new" ? `${defaultName}' created` : `${task?.title}' modified`} successfully`;
